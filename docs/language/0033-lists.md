...
A `List<T>` variable is a stable pointer to a heap-allocated 3-field header, `{i32 length, T* data, i32 capacity}`. `push` and `pop` mutate the header's own fields *in place* — the pointer itself never changes, even when `push` reallocates the backing buffer (only the header's `data`/`capacity` fields are updated to point at the new buffer). This is exactly the same shape that already makes struct field-assignment and array index-assignment write-through correctly: because the outward-facing pointer is stable, every existing alias to it sees a mutation immediately, with no extra plumbing needed anywhere.

`capacity` is deliberately the *last* field (index 2), not field 1 the way `Buffer`'s own `{length, capacity, data}` order puts it (see `docs/language/0043-buffer.md`) - field 1 has to stay `T* data`, unchanged from before this field existed, to avoid a real type collision: `{i32, i32, T*}*` (capacity at field 1) is byte-for-byte the same LLVM type text `Deque<T>`'s own `{count, start, data}` header already produces for the same element type (see `docs/language/0037-deques.md`) - `isDequeType`'s own structural check distinguishes collections purely by that prefix text, so `List<T>` and `Deque<T>` would become indistinguishable. Keeping `data` at field 1 means every List-reading site elsewhere in the backend (indexing, printing, slicing, `.join()`) needed zero changes when capacity was added - only `push`'s own growth logic, and anything that *constructs* a fresh header, needed to know about the new field at all.
...
`TypeKind::List` is new; `Type` originally reused the `elementKind`/`elementStructName` fields arrays and slices already added — no new `Type` fields needed at the time (later replaced by a single string-based `elementTypeName` field, shared with `Map<K,V>`/`Set<T>`'s own representation, to support arbitrary nesting — see `docs/language/0053-nested-generics.md`). `resolveType` recognizes `"List<elem>"` exactly like it already recognizes `"slice<elem>"` and `"[elem;N]"`. `isIndexable` (already `Array || Slice`) grows to `Array || Slice || List`, so `numbers[i]` and `numbers.length` type-check through the exact same code path arrays and slices already use — no new branch needed for either.
...
**Amortized (doubling) growth**, via a shared `ensureListCapacity` helper (mirroring `Buffer`'s own `ensureBufferCapacity` - see `docs/language/0043-buffer.md`): `push` only reallocates when `length + 1` would exceed the list's own current `capacity`, and when it does, it grows to `max(needed, capacity * 2)` - not to exactly `needed`, so most pushes don't reallocate at all. This was originally shipped as "reallocate to exactly `length + 1` on every push, O(n) per push, O(n²) total" - a deliberate first-pass simplification, called out explicitly at the time as "a pure internal implementation detail... a future capacity-doubling rewrite changes zero language surface." That rewrite is this. `capacity` starts at 0 on construction; the doubling formula's own `needed > capacity * 2` check (not a bare `capacity * 2`) is what keeps the very first push - `0 * 2 = 0` - from stalling, by falling back to `needed` itself whenever doubling alone wouldn't be enough. `pop` still never shrinks capacity, matching `std::vector::pop_back`'s own identical choice.
